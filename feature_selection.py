...
features = np.array(['GDP','CPI','Inflation','Tariff rate','Unemployment','Trade balance'])

num_runs = 100
d = {'GDP':0, 'CPI':0, 'Inflation':0, 'Tariff rate':0, 'Unemployment':0, 'Trade balance':0}

for i in range(num_runs):
    # Perform classification
    classifier = RandomForestClassifier()
    classifier.fit(df[features], df['FDI'])
